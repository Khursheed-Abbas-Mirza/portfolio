...
passport.use("github-signup",new Strategy({
    ...
    callbackURL:"https://portfolio-alpha-five-25.vercel.app/github/user/callback?action=signup",
...
passport.use("github-signin",new Strategy({
    clientID:process.env.CLIENT_ID,
    clientSecret:process.env.CLIENT_SECRET,
    callbackURL:"https://portfolio-alpha-five-25.vercel.app/github/user/callback?action=signin",
},async(accesstoken,refreshtoken,profile,done)=>{
    const username=profile.username
    const githubId=profile.id 
    const email=profile.emails?profile.emails[0].value:null
